...
extern struct pipe_desc* fb_inputs[];
extern struct xcode_desc* fb_jacks[];
extern struct device_desc fb_dev;

struct device_desc fb_dev={
	.desc.name	=	"frame buffer output device",
	.version		=	1,
	.index			=	"",
	.location		=	"",
	.jacks			= fb_jacks,
...
struct jack_desc fb_jack={
			.desc.name        =	"ml fb output jack",
			.desc.parent      =	(struct type_desc*) &fb_dev,
			.type             =,
			.direction        =,
			.size             =,
			.paths            =,
...
struct jack_desc* fb_jacks[]={&fb_jack};

struct path_desc fb_input={
	.desc.name        ="input stream",
	.desc.parent      =(struct type_desc*)&fb_jack,
	.type             =,
	.compAlignement   =,
	.bufferAlignement =,
	.dest             =&fb_jack,
	};
struct pipe_desc* fb_inputs[]={&fb_input};
